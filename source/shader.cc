...
bool compileShader(glslang::TShader* shader, const char* filename,
  ...
  config::initResources(resource);
  std::string content{};
  if (!helper::loadFile(filename, content)) {
    return false;
  }
  const char* const tempStrs[] = {content.c_str()};
  shader->setStrings(tempStrs, 1);
  shader->setEntryPoint("main");
  shader->setEnvInput(config::kSourceLanguage, static_cast<EShLanguage>(stage),
                      config::kClient, config::kGlslVersion);
  shader->setEnvClient(config::kClient, config::kClientVersion);
  shader->setEnvTarget(config::kTargetLanguage, config::kTargetLanguageVersion);
  if (!shader->parse(&resource, config::kGlslVersion, false,
                     config::kMessages)) {
...
struct Converter {
  ...
  vk::DescriptorSetLayoutBinding toDescriptor(
    ...
    if (!helper::getType(*ttype, binding.descriptorType)) {
 ...
 public:
  ShaderAnalyzer(std::map<EShLanguage, const char*> shaders) {
    ...
    for (const auto& e : shaders) {
      ...
      if (helper::compileShader(handle.get(), e.second, e.first)) {
    ...
    if (program->link(config::kMessages) && program->buildReflection()) {
    } else {
      throw std::exception("error link shader");
    }
  }
  ~ShaderAnalyzer() { glslang::FinalizeProcess(); }

  bool getObject(vk::Device& device, ShaderObject& object) const {
    vk::Result result = vk::Result::eSuccess;

    helper::Converter cvt{};
    ...
    for (const auto& shader : shaderTable) {
      ...
      bool hasStage = helper::stageCast(shader.first, stage);
      if (!hasStage) {
        return false;
      }
...
std::unique_ptr<ShaderObject> ShaderObject::createFromFiles(
  ...
  try {
    ...
    for (const auto& e : files) {
      tmpShaderFiles[helper::stageCast(e.first)] = e.second;
    }
