...
static std::string TrimStr(const std::string& src, char ch, bool left, bool right) {
    size_t start = 0;
    if (left)
...
static void SpiltStr(std::vector<std::string>& out, const std::string& src, char delm) {
    size_t start = 0;
    size_t pos = src.find('.');
    while (pos != src.npos) {
...
int Config::FindKey(const char* key) {
    ...
    std::string fmtString{};
    {
        ...
        for (size_t i = 0; i < len; i++) {
            if (helper::AllowChar(key[i]))
                fmtString.push_back(key[i]);
        }
